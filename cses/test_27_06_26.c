#include <stdio.h>
#include <stdbool.h>

typedef struct {int a; int b;} ii;
typedef int (*func_cmp)(ii, ii);

int cmp(ii x, ii y) {
    if (x.a != y.a) {
        return (x.a < y.a) ? -1 : 1;
    }
    return 0;
}

void SWAP(ii *, ii *);
void PART(ii [], int *, int *, func_cmp);
void SORT(ii [], int, int, func_cmp);
int LOWER_BOUND(ii [], int, int);
int VLOOKUP(ii [], int, int);

int main() {
    int n, x;
    scanf("%d %d", &n, &x);

    ii arr[n];

    for(int i = 0; i < n; i++) {
        scanf("%d", &arr[i].a);
        arr[i].b = i;
    }

    SORT(arr, 0, n - 1, cmp);

    int pos = VLOOKUP(arr, n, x);
    int k = (pos % 2) ? 0 : 1;
    int flag = 1;

    for(int i = 0; i < pos / 2 + k; i++) {
        int j = LOWER_BOUND(arr, n, x - arr[i].a);

        if(j != -1) {
            printf("%d %d", arr[i].b + 1, arr[j].b + 1);
            flag = 0;
            break;
        }
    }

    if(flag) {
        printf("IMPOSSIBLE");
    }

    return 0;
}

void SWAP(ii *x, ii *y) {
    ii temp = *x;
    *x = *y;
    *y = temp;
}

void PART(ii arr[], int *low, int *high, func_cmp compare) {
    int l = *low;
    int r = *high;
    int m = *low;

    ii val = arr[l + (r - l) / 2];

    while(m <= r) {
        int cmp_result = compare(arr[m], val);

        if(cmp_result == 0) {
            m++;
        }
        else if(cmp_result < 0) {
            SWAP(&arr[l], &arr[m]);
            l++;
            m++;
        }
        else {
            SWAP(&arr[m], &arr[r]);
            r--;
        }
    }

    *low = l;
    *high = r;
}

void SORT(ii arr[], int l, int r, func_cmp compare) {
    while(l < r) {
        int low = l;
        int high = r;
        PART(arr, &low, &high, compare);

        if(low - l < r - high) {
            SORT(arr, l, low - 1, compare);
            l = high + 1;
        }
        else {
            SORT(arr, high + 1, r, compare);
            r = low - 1;
        }
    }
}

int LOWER_BOUND(ii arr[], int n, int x) {
    int pos = -1;
    int l = 0;
    int r = n - 1;

    while(l <= r) {
        int m = l + (r - l) / 2;

        if(arr[m].a == x) {
            pos = m;
            r = m - 1;
        }
        else if(arr[m].a > x) {
            r = m - 1;
        }
        else {
            l = m + 1;
        }
    }

    return pos;
}

int VLOOKUP(ii arr[], int n, int x) {
    int pos = -1;
    int l = 0;
    int r = n - 1;

    while(l <= r) {
        int m = l + (r - l) / 2;

        if(arr[m].a <= x) {
            pos = m;
            l = m + 1;
        }
        else {
            r = m - 1;
        }
    }

    return pos;
}
